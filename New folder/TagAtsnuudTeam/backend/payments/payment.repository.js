...
// Payment select-ийг дахин ашигласнаар бүх query ижил response field name-тэй болно.
const paymentSelect = `
  SELECT
    id,
...
// Owner payout select мөн camelCase alias-уудтай.
const payoutSelect = `
  SELECT
    id,
    owner_id AS ownerId,
...
// Бүх payment-ийг хамгийн сүүлд үүссэнээс эхэлж авна.
async function getPayments() {
  const [rows] = await pool.execute(`${paymentSelect} ORDER BY created_at DESC`);
  return rows;
}

// Нэг payment record хайх helper.
...
// Payment үүсгээд шинэ record-ийг буцааж уншина.
...
// Payment status update хийхдээ paid үед paid_at автоматаар бөглөнө.
...
// Payment summary нь dashboard болон quick stats-д ашиглагдана.
...
// Commission report: нийт platform fee болон owner бүрийн revenue split.
...
// Payment бүрт payout давхар үүсэхээс сэргийлэхэд ашиглана.
...
// Owner payout үүсгээд шинэ record-ийг буцаана.
...
// Payout paid болсон үед paid_at автоматаар хадгална.
